...
markdown syntax in conversational responses inside the agent-shell, because
the comint buffer renders text as-is — `**bold**`, `| pipes |`, and `#
heading` all show as literal characters. Future contributors should not
paste markdown-heavy examples into the guidelines heredoc as model
demonstrations; use plain-text alignment (space-aligned columns,
indented definition lists) instead. Markdown remains correct for files,
PR bodies, commit messages, and any tool input that renders it.
