...
(declare-function decknix--live-sessions--write-file "decknix-agent-live-sessions")
(declare-function decknix--live-sessions--read-file "decknix-agent-live-sessions")
(declare-function decknix--live-sessions-filter-dismissed "decknix-agent-live-sessions")
...
(transient-define-prefix decknix-sidebar-transient--worktrees ()
  ...
  [:description "Worktree Toggles"
   ["Filter"
    (decknix-sidebar-transient--wt-age-filter)         ;; age (a)
    (decknix-sidebar-transient--wt-hide-clean)         ;; dirty-only (f)
    (decknix-sidebar-transient--wt-live-only)          ;; live-only (l)
    (decknix-sidebar-transient--wt-hide-merged)]       ;; merged (o)
   ["Display"
...
WIP / Sessions / Worktrees."
  ...
  [["WIP"
   ...
   ["Sessions"
    ...
    (decknix-sidebar-transient--sessions-age)          ;; age (a)
...
(defun decknix--sidebar-previous-sessions-save ()
  "Write the current `decknix--sidebar-previous-sessions' to disk.
Uses `decknix--sidebar-previous-sessions-file'.  Atomic write via
`decknix--live-sessions--write-file' (best-effort)."
  (when (and (boundp 'decknix--sidebar-previous-sessions-file)
             decknix--sidebar-previous-sessions-file)
    (decknix--live-sessions--write-file
     decknix--sidebar-previous-sessions-file
     decknix--sidebar-previous-sessions
     ";; Auto-generated by decknix-agent-shell-workspace — do not edit\n")))

(defun decknix--sidebar-previous-sessions-restore ()
  "Restore the Previous Sessions list from disk.
Reads `decknix--sidebar-previous-sessions-file' and populates the
in-memory list.  Used to restore state across hot-reloads."
  (when (and (boundp 'decknix--sidebar-previous-sessions-file)
             decknix--sidebar-previous-sessions-file)
    (let ((stored (decknix--live-sessions--read-file
                   decknix--sidebar-previous-sessions-file)))
      (when stored
        (setq decknix--sidebar-previous-sessions stored)))))
               ...
               (let ((choice (read-char-choice
                 ...
                 (pcase choice
                   ...
                   (?d
                    ...
                    ;; now that previous-sessions has its own snapshot
                    ;; file (restored across hot-reloads), the dismissal
                    ;; needs its own file too.
                    (let ((key
